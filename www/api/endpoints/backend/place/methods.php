<?php
    ...
    class Methods extends Base_Methods {
        ...
        #region Public Methods
            ...
            // Get
            public function get($idPlace, $isValid = 0) {
            ...
            public function getAll() {
                ...
                if (!Base_Functions::IsNullOrEmpty($request->IdsCategories)) {
                    ...
                    $categories_where = " AND (SELECT GROUP_CONCAT(cpp.IdCategory ORDER BY cpp.IdCategory)
                                        FROM categories_places_parents cpp
                                        WHERE cpp.IdPlace = p.IdPlace AND cpp.IdCategory IN($IdsCategories)) = '$IdsCategories'";
                }
                else
                    $categories_where = "";
                ...
                $body_query = "FROM places p
                INNER JOIN places_translations pt ON p.IdPlace = pt.IdPlace AND pt.IdLanguage = (SELECT MIN(pt2.IdLanguage)
                                                                                            FROM places_translations pt2
                                                                                            WHERE pt2.IdPlace = p.IdPlace
                                                                                            )
                WHERE " . str_replace("(t1)", "p.", $basic_where) . $categories_where;
                ...
                if (count($all) > 0) {
                    ...
                    foreach($all as $translations) {
                        ...
                        if(property_exists($categories_selected, $translation->IdPlace)) {

                            // Get the categories of the place
                            $place_categories_selected = array_column($categories_selected->{$translation->IdPlace}, "IdCategory");
                            // Get the main category of the place
                            $main_category_selected  = (count($place_categories_selected) > 0) ? $categories_selected->{$translation->IdPlace}[array_search(1, array_column($categories_selected->{$translation->IdPlace}, 'IsMain'))]->IdCategory : null;
            ...
            public function create() {
                ...
                $idPlace = $this->__opHelper->object($this->id)->table($this->table_name)->insertIncrement();

                // Check if created
                if(is_numeric($idPlace) && $idPlace > 0)
                    return $this->Success($idPlace);
            ...
            public function update() {
                ...
                if ($this->Success == true) {
                    ...
                    if($request->IdNews != null) {
                        $values = '';
                        $fields = '(IdPlace, IdNews)';
                        foreach($request->IdNews as $news) {
                            $values .= "({$request->IdPlace}, $news), ";
                        }
                        $values = rtrim($values, ', ');
                    ...
                    $sql = "DELETE FROM $this->table_translations_name WHERE $this->id = $request->IdPlace";
                    $this->__linq->queryDB($sql)->getResults();

                    // Insert the languages
                    $this->__opHelper->table($this->table_translations_name)->insertMassive("(IdPlace, Description, SmallDescription, IdLanguage, IsValid)", implode(", ", $translation_values));

                    // Delete the old traveler path answers
                    $sql = "DELETE FROM surveys_questions_answers_places WHERE IdPlace = $request->IdPlace";
                    $this->__linq->queryDB($sql)->getResults();

                    if (!Base_Functions::IsNullOrEmpty($request->IdSurveyQuestionAnswers)) {
                        
                        $values = "($request->IdPlace, " . implode("), ($request->IdPlace, ", $request->IdSurveyQuestionAnswers) . ")";
                        $this->__opHelper->table("surveys_questions_answers_places")->insertMassive("(IdPlace, IdSurveyQuestionAnswer)", $values);
                    }
                ...
                #region Images
                    ...
                    public function saveImages() {
                        ...
                        if ($this->Success == true) {
                            ...
                            // Save the images
                            Base_File::saveContentManager($request->IdPlace, Base_Files::PLACE, Base_Files_Types::IMAGE);
            ...
            #region Jobs
                ...
                public function deleteFilesAfter365days() {
                    ...
                    if (count($place) > 0) {
                        ...
                        // Get the IdPlace array
                        $ids_places = array_column($place, "IdPlace");

                        // Get the id place string
                        $ids_places_string = implode(", ", $ids_places);    

                        // Delete from images table
                        $sql = "DELETE FROM places_images WHERE IdPlace IN ($ids_places_string)";
                        $this->__linq->queryDB($sql)->getResults();

                        // Delete from translations table
                        $sql = "DELETE FROM places_translations WHERE IdPlace IN ($ids_places_string)";
                        $this->__linq->queryDB($sql)->getResults();
        ...
        #region Private Methods
            ...
            private function formatPlace($place, $place_translations = null, $place_images = null, $place_categories = null, $isAll = false) {
                ...
                if($place_translations == null)
                    $place_translations = $this->__linq->fromDB($this->table_translations_name)->whereDB("IdPlace = $place->IdPlace")->getResults();
                // get Image
                if($place_images == null)
                    $place_images = $this->__linq->selectDB("IdPlace, FullPath, FileName")->fromDB($this->table_images_name)->whereDB("IdPlace = $place->IdPlace")->getResults();

                // Reorder
                $place_translations = $this->__linq->reorder($place_translations, "IdPlace", true);
                $place_images = $this->__linq->reorder($place_images, "IdPlace", true);

                // Build object
                $response = new stdClass();
                $response->IdPlace = $place->IdPlace;
                ...
                $response->Image = (property_exists($place_images, $place->IdPlace)) ? $place_images->{$place->IdPlace}[0] : array();
                ...
                // Check translations
                if(property_exists($place_translations, $place->IdPlace)) {

                    if ($isAll) {

                        // Get translations
                        $response->Description = $place_translations->{$place->IdPlace}[0]->Description;
                        $response->SmallDescription = $place_translations->{$place->IdPlace}[0]->SmallDescription;
                        $response->LanguagesIds = array_column($place_translations->{$place->IdPlace}, "IdLanguage");
                    }   
                    else {

                        // Get translations
                        $place_translations = $place_translations->{$place->IdPlace};
                        ...
                        foreach ($place_translations as $translation) {
                            ...
                            if($isAll)
                                $obj->Categories = $this->getCategoryByLang($response->IdPlace, $obj->IdLanguage);
            ...
            private function getCategoryByLang($idPlace, $language) {
                $response = '';

                $categories = array_column($this->__linq->selectDB('IdCategory')->fromDB("categories_places_parents")->whereDB("IdPlace = $idPlace")->getResults(), 'IdCategory');
